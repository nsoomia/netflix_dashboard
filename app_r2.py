...
selection = st.sidebar.radio("이동할 단계를 선택하세요:", 
    ["Overview", "1단계: 선택의 과부하 검증", "2단계: 이탈의 임계점 분석", "3단계: 추천 전환 효과 분석"])
...
if selection == "Overview":
    ...
    ### 🎯 주요 분석 목표
    1. **선택의 과부하 검증**: 검색 결과가 많으면 검색 소요시간이 늘고, 클릭률이 떨어지는가?
    2. **이탈의 임계점 분석**: 검색 시간이 길어지면 언제 이탈하는가?
    3. **추천 전환 효과 분석**: 검색 실패(검색 결과 0건) 시 추천 콘텐츠는 효과가 있는가?
    """)
...
# --- 1단계: 선택의 과부하 검증 ---
    ...
    with tab3:
        ...
        st.markdown("""
        * **현상 및 인사이트:** 
            1. 검색 결과가 50개 이상이어도 사용자는 포기하지 않고 '빠른 스캔(Skimming)'으로 효율적 탐색을 수행함.
            2. 상위 1~3위(Top 3) 클릭 점유율이 29.4%로 낮음 (일반적으로 과반수여야 함). 
            3. 상단 콘텐츠 클릭 집중도를 50-60% 수준으로 향상할 수 있도록 랭킹 알고리즘 개선 필요.
        * **액션 플랜:**
...
# --- 2단계: 이탈의 임계점 분석 ---
    ...
    with tab3:
        st.success("""
        - ✅ 가설: 콘텐츠 탐색 시간이 증가할수록 콘텐츠 선택률이 낮아지며, 특정 탐색 시간 이후부터는 이탈률이 증가할 것이다.
        - ✅ 판단 결과: 진실 (True)
        """)
        st.subheader("💡 비즈니스 인사이트 & 액션플랜")
        st.markdown("""
        ...
        * **기대효과:**
            1. 이탈 임계점(28–48초) 이전에 개입함으로써 사전 이탈 방지 가능
            2. 고위험 구간만 선별 대응으로 UX 리소스 효율 극대화
        """)

# --- 3단계: 실패 극복 효과 측정 ---
elif selection == "3단계: 추천 전환 효과 분석":
    st.header("Phase 3: 추천 전환 효과 분석 (Recovery Analysis)")
    st.subheader("📊 분석 결과")
